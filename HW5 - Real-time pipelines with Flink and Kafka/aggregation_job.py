...
#Added this to sessionizes the input data by IP address and host
def create_aggregated_events_ip_sink_postgres(t_env):
    table_name = 'processed_events_aggregated_ip'
    sink_ddl = f"""
        CREATE TABLE {table_name} (
            event_hour TIMESTAMP(3),
            host VARCHAR,
...
def log_aggregation():
    ...
    try:
        ...
        aggregated_ip_table = create_aggregated_events_ip_sink_postgres(t_env)
        ...
        t_env.from_path(source_table).window(
            ...
            .execute_insert(aggregated_ip_table) \
            .wait()
